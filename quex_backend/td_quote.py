...
class TDQuoteBody:
    ...
    def serialize(self):
        res = (
            self.tcb_svn[:16].ljust(16, b"\x00")
...
class P256Signature:
    ...
    def deserialize(b):
        assert len(b) == 64
        return P256Signature(r=b[:32], s=b[32:])
...
class EnclaveReportBody:
    ...
    def serialize(self):
        return (
            self.cpu_svn[:16].ljust(16, b"\x00")
...
class QECertificationData:
    ...
    def deserialize(b):
        ...
        if data_type == 6:
            ...
            certification_data = b[6 : 6 + size]
        return QECertificationData(certification_data_type=data_type, size=size, certification_data=certification_data)
    ...
    def serialize(self):
        if self.certification_data_type == 6:
            ...
            cert_data = self.certification_data
        return self.certification_data_type.to_bytes(2, "little") + len(cert_data).to_bytes(2, "little") + cert_data
...
class QEAuthenticationData:
    ...
    def deserialize(b):
        ...
        assert len(b) >= size + 2
        return QEAuthenticationData(size=size, data=b[2 : 2 + size])
...
class QEReportCertificationData:
    ...
    def deserialize(b):
        ...
        return QEReportCertificationData(
            qe_report=report, qe_report_signature=signature, qe_authentication_data=authentication_data, qe_certification_data=certification_data
        )

    def serialize(self):
        return (
            self.qe_report.serialize() + self.qe_report_signature.serialize() + self.qe_authentication_data.serialize() + self.qe_certification_data.serialize()
        )
...
class P256PublicKey:
    ...
    def deserialize(b):
        assert len(b) == 64
        return P256PublicKey(x=b[:32], y=b[32:64])
...
class ECDSA256QuoteSignatureData:
    ...
    def serialize(self):
        return self.quote_signature.serialize() + self.ecdsa_attestation_key.serialize() + self.qe_certification_data.serialize()
...
class TDQuote:
    ...
    def deserialize(b):
        ...
        return TDQuote(quote_header=header, td_quote_body=body, quote_signature_data_len=sig_len, quote_signature_data=sig_data)

    def serialize(self):
        sig_data = self.quote_signature_data.serialize()
        return self.quote_header.serialize() + self.td_quote_body.serialize() + len(sig_data).to_bytes(4, "little") + sig_data
