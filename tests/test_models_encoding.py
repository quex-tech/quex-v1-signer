...
class TestModelsEncoding(unittest.TestCase):
    ...
    f3 = open(Path(__file__).parent.resolve() / "test_vectors" / "oracle_message_test_vectors.json")
    ethereum_oracle_message_vectors = json.load(f3)
    f4 = open(Path(__file__).parent.resolve() / "test_vectors" / "plutus_oracle_message_test_vectors.json")
    plutus_oracle_message_vectors = json.load(f4)
